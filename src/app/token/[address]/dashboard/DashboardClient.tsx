...
export function DashboardClient({ tokenAddress }: DashboardClientProps) {
    ...
    const handleRugPull = async () => {
        ...
        if (!response.ok) throw new Error('Failed to rug');

        // Update Local State immediately
        setTokenData(prev => ({ ...prev, status: 'rugged' }));
        setIsRugged(true); // Also set isRugged state directly
        ...
        setLiveStats(prev => ({
            ...
            price: 0,
            marketCap: 0,
            holders: 0 // User requested 0 holders on rug
        }));
        ...
        console.error(error);
        alert("Failed to execute rug pull: " + error.message);
    } finally {
        setIsLoading(false);
    }
};
